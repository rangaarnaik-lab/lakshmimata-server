...
# Results catchup queue empty — Concall catchup may start next.
_RESULTS_CATCHUP_IDLE = False
_RESULTS_CATCHUP_IDLE_LOGGED = False
# Concall/transcript catchup queue empty — PPT catchup may start next.
_CONCALL_CATCHUP_IDLE = False
...
def _filing_catchup_enabled() -> bool:
...
def _ppt_transcript_catchup_boosted() -> bool:
    """Backward-compatible alias — prefer transcript then PPT helpers."""
    return _transcript_catchup_boosted() or _ppt_catchup_boosted()
...
def _about_hard_pause_seconds() -> int:
    """About hard-stop when every API key is exhausted (default 1h)."""
    return max(600, int(os.getenv(
        'ABOUT_COMPANY_HARD_QUOTA_COOLDOWN_SECONDS', '3600')))  # 1h
...
def _gemini_pool_rotated() -> list[str]:
    """Batch key pool with round-robin start — spreads load across all keys."""
    global _GEMINI_RR_INDEX
    pool = list(_gemini_batch_key_pool())
    if not pool:
        return []
    start = _GEMINI_RR_INDEX % len(pool)
    _GEMINI_RR_INDEX += 1
...
def _gemini_env_key_var_names() -> list[str]:
    ...
    return sorted(
        ...
        if n.startswith('GEMINI_API_KEY') and (os.environ.get(n) or '').strip()
    )


async def _gemini_generate_rotating(session: aiohttp.ClientSession, *,
                                    model: str, body: dict, timeout_s: float,
                                    symbol: str, label: str, priority: bool = False):
    """Try each API key in the shared pool before giving up (429 → next key)."""
    ...
    for i, api_key in enumerate(keys):
        ...
        status, data, body_txt = await _gemini_generate(
            session, url, body, timeout_s, api_key=api_key, priority=priority)
        if status == 200:
            return status, data, body_txt, False, api_key
        ...
        if hard_quota:
            log.warning(
                ...
                + (f" — trying next key ({i + 2}/{len(keys)})" if i + 1 < len(keys) else ""))
            if i + 1 < len(keys):
                continue
        elif status == 429:
            all_hard_quota = False
            log.warning(
                f"⚠️ Gemini {label} for {symbol} — RPM 429 on "
                f"{_gemini_key_fingerprint(api_key)}"
                + (f" — trying next key ({i + 2}/{len(keys)})" if i + 1 < len(keys) else ""))
            if i + 1 < len(keys):
                await asyncio.sleep(max(5, int(os.getenv('GEMINI_429_BACKOFF_SECONDS', '15'))))
                continue
        else:
            all_hard_quota = False
            break
    return last_status, last_data, last_body, all_hard_quota and last_status == 429, None
...
async def extract_results_from_pdf(session: aiohttp.ClientSession, symbol: str, attachment_url: str):
    ...
    prompt = (
        ...
        "units - convert to Rs Crore from whatever unit the document actually uses: Rs Lakh "
    ...
    # Stage 2: rotate across all GEMINI_API_KEY* on 429 before pausing.
    try:
        gemini_body = {
            "contents": [{"parts": [
                {"inline_data": {"mime_type": "application/pdf", "data": pdf_b64}},
        ...
        status, data, body, all_keys_exhausted, _used_key = await _gemini_generate_rotating(
            session,
            model=model,
            body=gemini_body,
            timeout_s=gemini_timeout,
            symbol=symbol,
            label='results extraction',
        )
        if status != 200:
            pause = status in (429, 500, 502, 503, 504) or status >= 500
            hard_quota = status == 429 and (all_keys_exhausted or _gemini_quota_exhausted(body))
            if status == 429:
                log.warning(f"⚠️ Gemini results 429 for {symbol}"
                            + (" — HARD QUOTA (all keys)" if hard_quota else " — rate limit")
                            + f": {(body or '')[:180]}")
            else:
...
async def _concall_summary_loop(session: aiohttp.ClientSession):
    ...
    while True:
            ...
            if retry_on in ('1', 'true', 'yes', 'on'):
                ...
                if retry_rows:
                    ...
                    for rr in retry_rows:
                        already.discard((rr['symbol'], rr['attachment_url']))
                    pending.extend(retry_rows)
                    log.info(f"🎙️ Results catchup: re-queueing {retry_n} skipped/failed "
                              f"PDF(s) for symbols still missing financial_results")
        ...
        # Don't hand Gemini to Concall/PPT while a large Results-numbers gap remains.
        idle_missing_ok = int(os.getenv('RESULTS_CATCHUP_IDLE_MISSING_FR', '80'))
        progress_extra = (f"symbols_with_fr={len(have_fr)}, missing_fr≈{missing_fr_total}, "
                          f"universe≈{uni_n or '?'}, lookback={lookback_days}d"
                          f"{f', retry_skipped={retry_n}' if retry_n else ''}"
        ...
        for i, row in enumerate(todo):
            ...
            # Numbers without a prose summary still count as done — otherwise
            # catchup never retries and "Results PDF AI" under-counts.
            payload = {
                'symbol': row['symbol'], 'announced_at': row['announced_at'],
                'attachment_url': row['attachment_url'],
                'summary': summary,
                'status': 'done' if (summary or fd) else 'skipped',
            }
        ...
        if catchup:
            if todo:
                ...
                # Still missing numbers → re-scan soon for skipped PDF retries.
                # Only use the long "done" sleep once the FR gap is small.
                idle_missing_ok = int(os.getenv('RESULTS_CATCHUP_IDLE_MISSING_FR', '80'))
                if missing_fr_total > idle_missing_ok:
                    wait = int(os.getenv('RESULTS_PDF_CATCHUP_RETRY_SECONDS', '90'))
                    streak = getattr(_concall_summary_loop, '_idle_streak', 1)
                    if streak == 1 or streak % 10 == 0:
                        log.info(f"🎙️ Results catchup: waiting {wait}s then retry "
                                  f"(missing_fr≈{missing_fr_total}, lookback={lookback_days}d)")
...
async def _transcript_summary_loop(session: aiohttp.ClientSession):
    ...
    while True:
        ...
        if not _gemini_batch_key_pool():
            ...
            await asyncio.sleep(300)
            continue
        catchup = _transcript_catchup_boosted()
        BATCH_SIZE = int(os.getenv(
        ...
        if todo:
            ...
            _set_concall_catchup_idle(True)
...
async def extract_about_company(session: aiohttp.ClientSession, symbol: str, ppt_row, tx_row,
    ...
    try:
        status, data, body_txt, all_keys_exhausted, _used_key = await _gemini_generate_rotating(
            session,
            model=model,
            body=body,
            timeout_s=120,
        ...
        if status != 200:
            ...
            pause = status in (429, 500, 502, 503, 504) or status >= 500
            hard_quota = status == 429 and (all_keys_exhausted or _gemini_quota_exhausted(body_txt))
            log.warning(f"⚠️ Gemini about-company failed for {symbol} ({status})"
                        + (" — HARD QUOTA (all keys)" if hard_quota else "")
...
def _clean_mgmt_flags(v):
    """Normalize [{tone,title,detail}] flags from Gemini."""
    if isinstance(v, str):
        try:
            v = json.loads(v)
        except Exception:
    ...
    for item in v:
        if not isinstance(item, dict):
            continue
        tone = str(item.get('tone') or item.get('type') or '').strip().lower()
        if tone in ('green', 'positive', 'good'):
            tone = 'green'
        elif tone in ('red', 'negative', 'bad', 'risk'):
            tone = 'red'
        elif tone in ('watch', 'yellow', 'caution', 'amber'):
            tone = 'watch'
        else:
            continue
        title = str(item.get('title') or '').strip()[:120]
        detail = str(item.get('detail') or item.get('evidence') or '').strip()[:280]
        if len(title) < 3:
            continue
        out.append({'tone': tone, 'title': title, 'detail': detail or None})
    return out[:8] or None


def _clean_mgmt_verdict(v):
    allowed = ('trustworthy', 'mixed', 'caution', 'unknown')
    if not v:
        return 'unknown'
    t = str(v).strip().lower().replace(' ', '_')
    aliases = {
        'trustworthy': 'trustworthy', 'credible': 'trustworthy', 'good': 'trustworthy',
        'mixed': 'mixed', 'neutral': 'mixed', 'okay': 'mixed',
        'caution': 'caution', 'cautious': 'caution', 'weak': 'caution', 'untrustworthy': 'caution',
...
async def _gather_stock_ask_context(session, headers, symbol: str):
    ...
    try:
        async with session.get(
            f"{SUPABASE_URL}/rest/v1/ppt_summaries", headers=headers,
            params={'select': 'announced_at,attachment_url,overall_summary,business_segments,'
                    'strategic_initiatives,financial_highlights,risks_flagged,outlook_guidance,'
                    'emerging_themes,theme_evidence,management_tone,watch_next',
                    'symbol': f'eq.{symbol}', 'status': 'eq.done',
                    'order': 'announced_at.desc', 'limit': '2'},
            timeout=aiohttp.ClientTimeout(total=20),
        ) as r:
            rows = await r.json() if r.status == 200 else []
            ppt = rows[0] if isinstance(rows, list) and rows else None
        async with session.get(
            f"{SUPABASE_URL}/rest/v1/transcript_summaries", headers=headers,
            params={'select': 'announced_at,attachment_url,overall_summary,competitive_positioning,'
                    'expansion_capex,outlook_guidance,financial_highlights,risks_flagged,'
                    'key_concerns,emerging_themes,theme_evidence,management_tone,watch_next',
                    'symbol': f'eq.{symbol}', 'status': 'eq.done',
                    'order': 'announced_at.desc', 'limit': '2'},
            timeout=aiohttp.ClientTimeout(total=20),
        ) as r:
            rows = await r.json() if r.status == 200 else []
            tx = rows[0] if isinstance(rows, list) and rows else None
        async with session.get(
            f"{SUPABASE_URL}/rest/v1/company_abouts", headers=headers,
            params={'select': 'overall_brief,what_they_do,customers,segments,innovation,sources',
                    'symbol': f'eq.{symbol}', 'status': 'eq.done', 'limit': '1'},
            timeout=aiohttp.ClientTimeout(total=15),
        ) as r:
            rows = await r.json() if r.status == 200 else []
    ...
    if about:
        ...
        for k, label in (('segments', 'SEGMENTS'), ('innovation', 'INNOVATION'),
            ...
            if bullets:
                about_bits.append(label + ':\n- ' + '\n- '.join(bullets))
        if about_bits:
            parts.append('COMPANY BRIEF:\n' + '\n'.join(about_bits)[:5000])
    for label, row in (('INVESTOR PRESENTATION', ppt), ('EARNINGS CALL', tx)):
        ctx = _fmt_filing_context(label, row)
        if ctx:
            parts.append(ctx[:7000])
    return '\n\n'.join(parts), fund or {}, ppt, tx, about
...
async def extract_management_flags(session: aiohttp.ClientSession, symbol: str, context: str,
    ...
    try:
        ...
        return {
            ...
            'mgmt_flags': flags,
            'mgmt_flags_at': datetime.now(timezone.utc).isoformat(),
        }
    except Exception as e:
        log.warning(f"⚠️ Mgmt-flags error for {symbol}: {type(e).__name__}: {e}")
        return None
    ...
    while True:
        ...
        try:
            ...
            async with session.get(
                ...
                if r.status != 200:
                    body = await r.text()
                    if r.status == 400 and 'mgmt_flags' in body:
            ...
            for i, row in enumerate(todo):
                ...
                if not result:
                    continue
                try:
                    await save_fundamentals_batch_to_db(session, [{'sym': sym, **result}])
                    saved += 1
                    n = len(result.get('mgmt_flags') or [])
                    log.info(f"  🚩 {sym}: verdict={result.get('mgmt_verdict')} flags={n}")
                except Exception as e:
                    log.warning(f"⚠️ Mgmt-flags save failed for {sym}: {type(e).__name__}: {e}")
            log.info(f"🚩 Mgmt-flags loop: saved {saved}/{len(todo)}")
        except Exception as e:
            log.error(f"Mgmt-flags loop failed: {type(e).__name__}: {e}")
        await asyncio.sleep(CHECK_INTERVAL)
        ...
        if need_cooldown or all_failed:
            ...
            if hit_hard_quota or _ABOUT_GEMINI_HARD_QUOTA:
                ...
                _begin_about_yield_to_results(
                    hard_cool,
                    f"HARD QUOTA all keys (ok={gemini_ok}, errors={gemini_errors}/{len(todo)}); "
                    f"check AI Studio billing or add more GEMINI_API_KEY*")
...
def _gemini_focus_allows(feature: str) -> bool:
    ...
    Priority chain (automatic handoff):
      ...
      3) PPT catchup
    Ask-AI is never parked by this chain.
