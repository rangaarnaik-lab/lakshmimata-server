...
async def run_scan(session: aiohttp.ClientSession, scan_type: str = 'live') -> int:
    ...
    for loop_idx, s in enumerate(stocks_with_hist):
        ...
        processed.append({
            ...
            'pb':                 fundamentals_cache.get(sym, {}).get('pb'),
            'roce':               fundamentals_cache.get(sym, {}).get('roce'),
            'industry_pe':        fundamentals_cache.get(sym, {}).get('industry_pe'),
            'div_yield':          fundamentals_cache.get(sym, {}).get('div_yield'),
            'cfo':                fundamentals_cache.get(sym, {}).get('cfo'),
            'fcf':                fundamentals_cache.get(sym, {}).get('fcf'),
            'cfo_pat':            fundamentals_cache.get(sym, {}).get('cfo_pat'),
            'nim':                fundamentals_cache.get(sym, {}).get('nim'),
            'gnpa':               fundamentals_cache.get(sym, {}).get('gnpa'),
            'nnpa':               fundamentals_cache.get(sym, {}).get('nnpa'),
            'car':                fundamentals_cache.get(sym, {}).get('car'),
            'casa':               fundamentals_cache.get(sym, {}).get('casa'),
    ...
    # collisions with squeeze/VCP and across multiple signals on one stock.
    ...
    if new_signal_fires:
        log.info(
            f"  🔊 {len(new_signal_fires)} NEW signal fires: "
    ...
    if need_eod_archive:
        ...
        # for up to ~25 minutes (seen 2026-08-14 after archive fix).
        if scan_type == 'batch_eod':
            try:
