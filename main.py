...
async def seed_index_history_if_needed(session: aiohttp.ClientSession):
    """One-time seed: push 1492 days of Nifty 50 history to Supabase if DB is empty."""
    existing = await load_index_history_from_db(session, "Nifty 50")
    if len(existing) >= 1000:
        log.info(f"✅ Nifty 50 already seeded: {len(existing)} days in DB")
        return
    # Merge seed with any existing data
    if existing and len(existing) > 0:
        # Append existing (more recent) to seed
        merged = NIFTY50_SEED_PRICES + existing[-(len(existing)):]
        # Deduplicate by keeping unique sequential prices
        merged = list(dict.fromkeys(merged))
    else:
        merged = NIFTY50_SEED_PRICES
